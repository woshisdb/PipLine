...
#pragma warning restore 0649
...
#else
				( type.GetTypeInfo().IsGenericType && type.GetGenericTypeDefinition() == typeof( Dictionary<,> ) );
#endif
		}
		...
		protected override void OnBound( MemberInfo variable )
		{
			...
			Type[] genericArgs = BoundVariableType.GetGenericArguments();
			Type keyType = genericArgs[0];
			Type valueType = genericArgs[1];
			elementType = typeof(Tp<,>).MakeGenericType(keyType, valueType);
		}
		...
		protected override void GenerateElements()
		{
			...
			else
			{
				...
				foreach (var key in dictionary.Keys)
				{
					...
					var variableName = $"Key:{key}";
					var currentKey = key;
					elementDrawer.BindTo(elementType, variableName, () => Activator.CreateInstance(elementType,currentKey, dictionary[currentKey]), (value) =>
					{
						dictionary[currentKey] = value;
						Value = dictionary;
					});

					if (i < elementsExpandedStates.Count && elementsExpandedStates[i] && elementDrawer is ExpandableInspectorField expandableDrawer)
